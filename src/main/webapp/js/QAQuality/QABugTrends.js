...
$(drawChart=function(){
	seriesOptions=[],
	seriesCounter=0,
	severity=['1-Critical','2-Major','3-Average','4-Minor'];
	Highcharts.setOptions({
		global:{
			useUTC:false,
		},
		colors:[ '#da4f49','#006dcc','#faa732','#49afcd', '#363636','#5bb75b']
	});
	...
	function columnClick(event){
		...
		$.ajax({
				...
				data:'severity='+severity+'&time='+time+'&datarange='+datarange+"&domain="+domain,
		...
		var chart = new Highcharts.Chart({
			...
			tooltip:{
				...
				formatter: function() {													
				    ...
				    if(datarange=="week"){
				    	var weekLaterTime = new Date(this.x+518400000);  // add 6 days
				    	var weekLaterTimeStr = weekLaterTime.toDateString();
				    	var weekLaterMonth = weekLaterTimeStr.split(" ")[1];
				    	var weekLaterDay = weekLaterTimeStr.split(" ")[2];
				    	var weekLaterYear = weekLaterTimeStr.split(" ")[3];
				    	result += month + '/' + day + '/' + year + ' - ' + weekLaterMonth + '/' + weekLaterDay + '/' + weekLaterYear;
				    } else if(datarange=="month"){		
						result += month + '/' + year;	
					} else if(datarange=="quater"){		
					    var quarter = '';
					...
					result += '<br/>Total: '+ this.point.stackTotal;
	...
	function getData(){
		seriesCounter=0;
		$.each(severity,function(i,name){
			$.getJSON('QAQuality!QABugTrend.action?domain='+domain+'&severity='+name+'&datarange='+datarange+'&from='+from+'&to='+to,function(data){
				...
				seriesOptions[i] = {
	   		    ...
	   		    seriesCounter += 1;
	            if (seriesCounter == severity.length) {
	            	console.log(seriesOptions);
