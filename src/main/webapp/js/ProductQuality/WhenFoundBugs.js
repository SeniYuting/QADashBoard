...
$(drawChart=function(){
	seriesOptions=[],
	seriesCounter=0,
	when=['Before-Release','Beta-Testing','Post-Release'];
	...
	function columnClick(event){
		var when=event.point.series.name;
		var time=event.point.category;
		$.ajax({
				url:'Detail!getProductionDetail.action',
				data:'when='+when+'&time='+time+'&datarange='+datarange+'&domain='+domain,
	...
	function getData(){
		seriesCounter=0;
		$.each(when,function(i,name){
			$.getJSON('ProductionQuality!WhenFoundBug.action?domain='+domain+'&when='+name+'&datarange='+datarange+'&from='+from+'&to='+to,function(data){
